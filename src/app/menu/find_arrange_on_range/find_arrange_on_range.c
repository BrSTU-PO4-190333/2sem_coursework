#include "find_arrange_on_range.h"

void find_arrange_on_range(struct reservoirs_of_the_country *array, int length)
{
    clear_console();

    if (length == 0)
    {
        printf("Нет элементов для вычисления среднего значения\n");
    }
    else
    {
        int left_border = -1;
        ...
        out_average_values_in_line(array, left_border, right_border);
